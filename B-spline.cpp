...
double BSpline::basic(int i, int k, double t)
{
    double value;
    if (k == 1)
...
double BSpline::basic(int i, int degree, double t, double* bp)
{
    double temp = 0;
...
void BSpline::computePoint(std::vector<cv::Point> control, cv::Point* output, cv::Point* slope, double* mat_ptr, double t, int degree)
{
    double b = 0, bp = 0;
...
BSpline::BSpline(int n, int resolution, std::vector<cv::Point> control, std::vector<cv::Point>& result)
    :basic_mat_(cv::Mat(resolution, control.size(), CV_64FC1)),
    knots(std::vector<int>(control.size() + n, 0)),
    curve_((n > 0 && resolution > 0) ? new cv::Point[resolution] : NULL),
    tangent_((n > 0 && resolution > 0) ? new cv::Point[resolution] : NULL)
{
    double increment, interval;
    cv::Point tmp_point, tmp_tangent;
